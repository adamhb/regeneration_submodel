...
for (fl in files){
  ...
  tmp_ED2_data <- data.frame(pft_tmp = pft,
                             ba_per_cohort = BA.CO.tmp0, #cm2 per cohort
                             patchID = tmp1, 
  ...
  tmp_ED2_data2 <- tmp_ED2_data1 %>%
    group_by(pft) %>%
    summarise(ba_per_pft = sum(ba_per_cohort),
              npp_pft_mo = sum(npp_co_mo), #g C per pft per month
...
input_data <- ED2_data_daily2 %>%
  dplyr::select(yr, month, day, pft, date, dbh, N_co, ba_per_pft, SMP, NPP, c_repro, FSDS, nppseed_pft_day) %>%
