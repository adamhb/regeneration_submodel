...
source("model_dev/RA_curves_from_Visser.R")
...
if(from_new_data == T){
  ...
  for(r in runs){
    ...
    write_csv(input_data %>% select(yr,pft,ba_per_pft) %>% mutate(run = basename(driver_data_path)),path = paste0("temp/",basename(driver_data_path),"ba.csv"))
...
BA_fig <- BAdata2 %>%
  ggplot(aes(simYr,ba_per_pft,color = pft)) +
  geom_smooth(se = F) +
  ...
  geom_point(data = total_ba_per_pft_at_bci,mapping = aes(x = 15,y = ba.gr.pft / 50),size = 3) +
  adams_theme
...
makePNG(BA_fig,path_to_output.x = path_to_MS_figures,file_name = "NPP_figs")
