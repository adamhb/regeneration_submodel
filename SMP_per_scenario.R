...
if(from_new_data == T){
  ...
  for(r in runs){
    ...
    write_csv(input_data %>% select(yr,month,pft,NPP,SMP) %>% mutate(run = basename(driver_data_path)),path = paste0("temp/",basename(driver_data_path),".csv"))
...
NPPdata <- read_csv("temp/ED2_output.csv") %>%
  ...
  rbind(read_csv("temp/analy_dry75.csv")) %>% str()
  left_join(precip_data, by = c("yr","month","run"))

  
SMP_data <- read_csv("temp/ED2_output.csv") %>%
  rbind(read_csv("temp/ED2_ENSO.csv")) %>%
  rbind(read_csv("temp/ED2_wet.csv")) %>%
  rbind(read_csv("temp/analy_dry75.csv"))

SMP_data %>% group_by(run, pft) %>% summarize(x = length(SMP))

day <- rep(1:(56520/4),16)


# SMP_data %>%
#   add_column(day = day) %>%
#   filter(pft == "LD_DT") %>%
#   #filter(run == "ED2_ENSO", pft == "LD_DT") %>%
#   #mutate(x = row_number()) %>% pull(row)
#   ggplot(aes(day,SMP/1e5)) +
#   geom_line() +
#   facet_wrap(~run) 
#   
...
vh_line <- data.frame(
  ...
  makePNG(SMP_fig,path_to_output.x = path_to_MS_figures,file_name = "SMP_figs")
...
NPPdata %>%
  rownames_to_column(var = "day") %>%
  filter(as.numeric(day) %% 200 == 0, pft == "ST_DT") %>%
  mutate(simYr = as.numeric(stringr::str_sub(yr, start = 1, end = 4)) - 2000) %>%
  mutate(run = case_when(
    run == "analy_dry75" ~ "DRY-DS",
    run == "ED2_ENSO" ~ "SYN-ENSO",
