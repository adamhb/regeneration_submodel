...
for (fl in files){
  ...
  MMEAN_NPP_BG = mydata1[["MMEAN_NPPCROOT_CO"]][] + mydata1[["MMEAN_NPPFROOT_CO"]][]
  ...
  tmp_ED2_data <- data.frame(pft_tmp = pft, 
                             ...
                             npp_co_per_ind_bg = MMEAN_NPP_BG,
  ...
  tmp_ED2_data1 <- tmp_ED2_data %>%
    ...
    mutate(npp_co_mo_bg = npp_co_per_ind_bg * 1000 * nplant_per_co / (12 * 10000)) %>% # same units as above
  ...
  tmp_ED2_data2 <- tmp_ED2_data1 %>%
    ...
    summarise(npp_pft_mo = sum(npp_co_mo),#g C per pft per month
              npp_pft_mo_bg = sum(npp_co_mo_bg),
...
ED2_data_daily1 <- ED2_data_daily %>% 
  mutate(npp_pft_day = npp_pft_mo / lubridate::days_in_month(Date),
         npp_pft_day_bg = npp_pft_mo_bg / lubridate::days_in_month(Date),
  ...
  rename(date = Date, NPP = npp_pft_day, NPP_bg = npp_pft_day_bg, N_co = n_per_pft, dbh = dbh_pft, c_repro = c_repro_per_pft_per_day)
...
input_data <- ED2_data_daily2 %>%
  dplyr::select(yr, month, day, pft, date, dbh, N_co, SMP, NPP, NPP_bg, c_repro, FSDS, nppseed_pft_day) %>%
...
mean_total_npp <- input_data %>%
  group_by(date) %>%
  summarise(npp = sum(NPP), npp_bg = sum(NPP_bg)) %>% pull(npp) %>% mean()

mean_bg_npp <- input_data %>%
  group_by(date) %>%
  summarise(npp = sum(NPP), npp_bg = sum(NPP_bg)) %>% pull(npp_bg) %>% mean()

mean_ANPP <- mean_total_npp - mean_bg_npp
mean_ANPP
