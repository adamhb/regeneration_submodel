...
forComparisonWithFATEStrs <- full_output %>% select(date, pft, NPP, c_repro, seedbank, seedpool, R) %>%
  mutate_at(.vars = "date",.funs = as.Date) %>%
  mutate_at(.vars = "NPP", .funs = function(x){x * (1/g_per_kg) * (1/sec_per_day)}) %>% #convert from g C m-2 day-1 to kg m-2 s-1
  mutate_at(.vars = "c_repro", .funs = function(x){x * (1/g_per_kg) * has_per_m2 * (1/sec_per_day)}) %>% #convert from g ha-1 day-1 to kg m-2 s-1
  mutate_at(.vars = c("seedbank","seedpool"),.funs = function(x){x * (1/g_per_kg) * has_per_m2}) %>% # converting from g ha-1 to kg m-2
  mutate_at(.vars = "R",.funs = function(x){x * has_per_m2 * days_per_yr}) %>% #converting from N recruits ha-1 day-1 to N recruits m-2 yr-1
  select(-pft) %>%
  group_by(date) %>% summarise_all(.funs = sum)

write_csv(forComparisonWithFATEStrs, 'temp/forComparisonToFATES.csv')
