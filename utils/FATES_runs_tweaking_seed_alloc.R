...
yearend     = 1903   ## end year of the simulation
...
#prefix1     = "testing1pft_nc.Cfe16302fc-F7c065e21."

runs <- c("default","1half_seed_alloc")
prefs <- c("testing1pft_nc.Cfe16302fc-F7c065e21.2020-06-18.clm2.h0.","1pft_half_seed_alloc.Cfe16302fc-F7c065e21.2020-06-25.clm2.h0.")   
inpaths <- c("~/cloud/gdrive/rec_submodel/data/FATES_data/home/adamhb/scratch/clmed-cases/testing1pft_nc.Cfe16302fc-F7c065e21.2020-06-18/run/lnd/hist/","~/cloud/gdrive/rec_submodel/data/FATES_data/home/adamhb/scratch/clmed-cases/1pft_half_seed_alloc.Cfe16302fc-F7c065e21.2020-06-25/run/lnd/hist/")
...
NPP      = NULL
RECRUITMENT = NULL
DDBH_CANOPY_SCPF = NULL
DDBH_UNDERSTORY_SCPF = NULL
MORTALITY_CANOPY_SCAG = NULL
MORTALITY_UNDERSTORY_SCAG = NULL
...
for (year in yearstart:yearend){
  for (month in 1:12){ ## comment this and uncomment next line to read only the same month from each year. Will speed-up reading very long time series.
    ...
    NPP[m]      = ncvar_get(mydata, "NPP")
    RECRUITMENT[m] = ncvar_get(mydata, "RECRUITMENT")
    DDBH_CANOPY_SCPF[m] = ncvar_get(mydata, "DDBH_CANOPY_SCPF")
    DDBH_UNDERSTORY_SCPF[m] = ncvar_get(mydata, "DDBH_UNDERSTORY_SCPF")
    MORTALITY_CANOPY_SCAG[m] = ncvar_get(mydata, "MORTALITY_CANOPY_SCAG")
    MORTALITY_UNDERSTORY_SCAG[m] = ncvar_get(mydata, "MORTALITY_UNDERSTORY_SCAG")
...
outdat  = data.frame("date"   = thismonth,
                     ...
                     "DDBH_CANOPY_SCPF" = DDBH_CANOPY_SCPF,
                     "DDBH_UNDERSTORY_SCPF" = DDBH_UNDERSTORY_SCPF,
                     "MORTALITY_UNDERSTORY_SCAG"    = MORTALITY_UNDERSTORY_SCAG,
                     "MORTALITY_CANOPY_SCAG"  = MORTALITY_CANOPY_SCAG)

outdat <- outdat %>% add_column(run = run)

full_output <- rbind(full_output,outdat)

}

library(lubridate)

vars <- c("RECRUITMENT","DDBH_CANOPY_SCPF","DDBH_UNDERSTORY_SCPF","MORTALITY_UNDERSTORY_SCAG","MORTALITY_CANOPY_SCAG")


plot_FATES_output <- function(y, x = "date", dat = full_output, create_PNG = F) {
  ...
  if(y == "RECRUITMENT"){
    ylab_units <- "(indiv / ha / yr)"
  }
  
  if(y == "DDBH_CANOPY_SCPF" | y == "DDBH_UNDERSTORY_SCPF"){
    ylab_units <- "(cm / yr / ha)"
  }
  
  if(y == "MORTALITY_UNDERSTORY_SCAG" | y == "MORTALITY_CANOPY_SCAG"){
    ylab_units <- "(plants / ha / yr)"
  }else{ylab_units <- ""}

  png(filename = paste0(outpath,y,".png"))
  print({
    ggplot(dat, aes(x = lubridate::mdy(.data[[x]]), y = .data[[y]], color = .data[["run"]]) ) +
      geom_point() +
      #geom_smooth(method = "loess", se = FALSE, color = "grey74") +
      theme_bw() +
      scale_color_manual(values = c("blue","black")) +
      labs(x = x,
           y = paste(y,ylab_units),
           title = y) 
  })
  dev.off()
  
  # output_plot <- ggplot(dat, aes(x = lubridate::mdy(.data[[x]]), y = .data[[y]], color = .data[["run"]]) ) +
  #   geom_point() +
  #   #geom_smooth(method = "loess", se = FALSE, color = "grey74") +
  #   theme_bw() +
  #   scale_color_manual(values = c("blue","black")) +
...
#plot_FATES_output(y = "RECRUITMENT")


FATES_plots = map(.x = vars,.f = plot_FATES_output)


# summary(full_output$DDBH_CANOPY_SCPF)
# full_output %>%
# ggplot(aes(x = date, y = DDBH_CANOPY_SCPF, color = run) ) +
#   geom_point() +
#   #geom_smooth(method = "loess", se = FALSE, color = "grey74") +
#   theme_bw() +
#   scale_color_manual(values = c("blue","black")) 
